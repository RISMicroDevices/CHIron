...
#ifndef __CCHI__CCHI_XACT_XACTIONS_IMPL__CACHEABLE_WRITE
#define __CCHI__CCHI_XACT_XACTIONS_IMPL__CACHEABLE_WRITE
...
namespace CCHI::Xact {
    ...
    class XactionCacheableWrite : public Xaction<config> {
    public:
        XactionCacheableWrite(const Global<config>&             glbl,
                              const FiredRequestFlit<config>&   first) noexcept;

    public:
        virtual std::shared_ptr<Xaction<config>>                Clone() const noexcept override;
        std::shared_ptr<XactionCacheableWrite<config>>          CloneAsIs() const noexcept;
...
// Implementation of: class XactionCacheableWrite
namespace CCHI::Xact {

    template<FlitConfigurationConcept config>
    inline XactionCacheableWrite<config>::XactionCacheableWrite(
        const Global<config>&               glbl,
        const FiredRequestFlit<config>&     first) noexcept
        : Xaction<config>(XactionType::CacheableWrite, first)
    {
        ...
        ) [[unlikely]]
        {
            this->firstDenial = this->RequestFlitDenied(XactDenial::DENIED_REQ_OPCODE, this->first,
                "This Opcode is not type of / supported by Cacheable Write transaction");
            return;
        }
    }

    template<FlitConfigurationConcept config>
    inline std::shared_ptr<Xaction<config>> XactionCacheableWrite<config>::Clone() const noexcept
    {
        return std::make_shared<XactionCacheableWrite<config>>(*this);
    }

    template<FlitConfigurationConcept config>
    inline std::shared_ptr<XactionCacheableWrite<config>> XactionCacheableWrite<config>::CloneAsIs() const noexcept
    {
        return std::make_shared<XactionCacheableWrite<config>>(*this);
    }

    template<FlitConfigurationConcept config>
    inline bool XactionCacheableWrite<config>::GotDBIDResp() const noexcept
    {
        return this->HasDnRSP({ Opcodes::DnRSP::DBIDResp, Opcodes::DnRSP::CompDBIDResp });
    }

    template<FlitConfigurationConcept config>
    inline bool XactionCacheableWrite<config>::GotComp() const noexcept
    {
        return this->HasDnRSP({ Opcodes::DnRSP::Comp, Opcodes::DnRSP::CompDBIDResp });
    }

    template<FlitConfigurationConcept config>
    inline bool XactionCacheableWrite<config>::GotAnyNonCopyBackWrData() const noexcept
    {
        return this->HasDnDAT({ Opcodes::UpDAT::NonCopyBackWrData });
    }

    template<FlitConfigurationConcept config>
    inline bool XactionCacheableWrite<config>::GotAllNonCopyBackWrData() const noexcept
    {
    ...
    template<FlitConfigurationConcept config>
    inline bool XactionCacheableWrite<config>::IsResponseComplete(const Global<config>& glbl) const noexcept
    {
        return GotDBIDResp() && GotComp();
    }

    template<FlitConfigurationConcept config>
    inline bool XactionCacheableWrite<config>::IsDataComplete(const Global<config>& glbl) const noexcept
    {
        return GotAllNonCopyBackWrData();
    }

    template<FlitConfigurationConcept config>
    inline bool XactionCacheableWrite<config>::IsTxnIDComplete(const Global<config>& glbl) const noexcept
    {
        return IsResponseComplete(glbl);
    }

    template<FlitConfigurationConcept config>
    inline bool XactionCacheableWrite<config>::IsDBIDComplete(const Global<config>& glbl) const noexcept
    {
        return IsDataComplete(glbl);
    }

    template<FlitConfigurationConcept config>
    inline bool XactionCacheableWrite<config>::IsComplete(const Global<config>& glbl) const noexcept
    {
        return IsResponseComplete(glbl) && IsDataComplete(glbl);
    }

    template<FlitConfigurationConcept config>
    inline XactDenialEnum XactionCacheableWrite<config>::NextDnRSPNoRecord(const Global<config>& glbl, const FiredResponseFlit<config>& dnrspFlit, bool& hasDBID, bool& firstDBID) noexcept
    {
        ...
        return this->ResponseFlitDenied(XactDenial::DENIED_DNRSP_OPCODE, dnrspFlit,
            "This DnRSP Opcode is not type of / supported by Cacheable Write transaction");
    }

    template<FlitConfigurationConcept config>
    inline XactDenialEnum XactionCacheableWrite<config>::NextUpRSPNoRecord(const Global<config>& glbl, const FiredResponseFlit<config>& uprspFlit, bool& hasDBID, bool& firstDBID) noexcept
    {
        return this->ResponseFlitDenied(XactDenial::DENIED_CHANNEL_UPRSP, uprspFlit,
            "Not expecting UpRSP flits for Cacheable Write transactions");
    }

    template<FlitConfigurationConcept config>
    inline XactDenialEnum XactionCacheableWrite<config>::NextDnDATNoRecord(const Global<config>& glbl, const FiredResponseFlit<config>& dndatFlit, bool& hasDBID, bool& firstDBID) noexcept
    {
        return this->ResponseFlitDenied(XactDenial::DENIED_CHANNEL_DNDAT, dndatFlit,
            "Not expecting DnDAT flits for Cacheable Write transactions");
    }

    template<FlitConfigurationConcept config>
    inline XactDenialEnum XactionCacheableWrite<config>::NextUpDATNoRecord(const Global<config>& glbl, const FiredResponseFlit<config>& updatFlit, bool& hasDBID, bool& firstDBID) noexcept
    {
        if (this->IsComplete(glbl))
        ...
        return this->ResponseFlitDenied(XactDenial::DENIED_UPDAT_OPCODE, updatFlit,
            "This UpDAT Opcode is not type of / supported by Cacheable Write transaction");
    }
}


#endif // __CCHI__CCHI_XACT_XACTIONS_IMPL__CACHEABLE_WRITE
