...
Options:
  ...
  --qt-root <path>      Qt prefix such as ~/Qt/6.11.0/macos or $(brew --prefix qt).
  --qt6-dir <path>      Direct path to Qt6's CMake package directory.
  --deploy-qt           Deploy the Qt runtime into the staged app bundle.
...
assert_qt_deployment() {
  ...
  local app_bundle="${release_root}/CloganGUI.app"
  local missing=()

  if [[ ! -d "$app_bundle" ]]; then
    missing+=("CloganGUI.app")
  fi
  if [[ ! -f "${app_bundle}/Contents/MacOS/CloganGUI" ]]; then
    missing+=("CloganGUI.app/Contents/MacOS/CloganGUI")
  fi
  if [[ ! -e "${app_bundle}/Contents/Frameworks/QtCore.framework" ]]; then
    missing+=("CloganGUI.app/Contents/Frameworks/QtCore.framework")
  fi
  if [[ ! -f "${app_bundle}/Contents/PlugIns/platforms/libqcocoa.dylib" ]]; then
    missing+=("CloganGUI.app/Contents/PlugIns/platforms/libqcocoa.dylib")
  fi
...
printf '\nBuild complete: %s/CloganGUI.app\n' "$release_dir"
